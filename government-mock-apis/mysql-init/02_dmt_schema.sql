...
--  (Motor Traffic Act, Chapter 203, Sections 124, 126(1), 128(1))
-- ============================================================
...
-- ────────────────────────────────────────────────────────────
--  REFERENCE TABLE: vehicle_class_definitions
--
--  Purpose: Stores the 14 official DMT vehicle class codes as
--  defined by the Motor Traffic Act. This is a fixed, read-only
--  lookup/dictionary table — it describes WHAT each class code
--  means. It is populated once at startup and never modified
--  by user actions.
--
--  WHY THIS IS SEPARATE FROM licence_vehicle_classes:
--  Without this table, the full description "All Cars/Dual
--  Purpose - GVW < 3500kg, Passengers < 8, Trailer < 250kg"
--  would need to be copied into every row of every person who
--  holds class B — that is data redundancy. Instead, we store
--  just the code 'B' in the person's record and JOIN here to
--  get the description on demand.
-- ────────────────────────────────────────────────────────────
...
-- Seed: 14 official classes from DMT Form MTA 30/2
...
('C',   'Motor Lorry - GVW > 17000kg, Trailer < 750kg',
        'Heavy Vehicle',        21),
('CE',  'Heavy Motor Lorry - GVW > 17000kg, Trailer > 750kg',
        'Heavy Vehicle',        21),
('D1',  'Light Motor Coach - Passengers < 32, Trailer < 750kg',
        'Passenger Transport',  21),
('D',   'Motor Coach - Passengers > 32, Trailer < 750kg',
        'Passenger Transport',  21),
('DE',  'Heavy Motor Coach - Trailer > 750kg',
        'Passenger Transport',  21),
...
('J',   'Special Purpose Vehicle',
        'Special',              21),
('PT',  'Public Transport Endorsement',
        'Endorsement',          23);


-- ────────────────────────────────────────────────────────────
--  FACT TABLE: driving_licences
--
--  One row per person's driving licence record.
--  Licence number is nullable (may not be issued yet).
-- ────────────────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS driving_licences (
    -- Surrogate UUID primary key
    id                  CHAR(36)        NOT NULL PRIMARY KEY,

    -- Nullable: licence number may not be assigned until after processing
    licence_number      VARCHAR(20)     UNIQUE DEFAULT NULL,

    -- NIC links to DRP citizen and is defined as UNIQUE to allow reference mapping
    nic                 VARCHAR(12)     NOT NULL UNIQUE,

    -- Name fields (from Section A of form)
    surname             VARCHAR(150)    NOT NULL,
    other_names         VARCHAR(150)    DEFAULT NULL,
    full_name_on_card   VARCHAR(255)    NOT NULL,

    -- Personal details (Section A & B of form)
    ...
    driver_restrictions ENUM('NONE', 'CORRECTIVE_LENSES', 'ARTIFICIAL_LIMB')
                                        NOT NULL DEFAULT 'NONE',

    -- File paths for binary assets stored on disk
...
-- ────────────────────────────────────────────────────────────
--  JUNCTION TABLE: licence_vehicle_classes
--
--  Purpose: Records WHICH vehicle classes have been granted on
--  a specific driving licence, along with the issued and expiry
--  date FOR EACH CLASS individually.
--
--  WHY THIS IS SEPARATE FROM driving_licences:
--  A single licence can authorise multiple vehicle classes (e.g.
--  B, B1, G1). Each class may have been issued on a different
--  date and may expire on a different date. Storing 14 pairs of
--  date columns directly on driving_licences would be unworkable.
--  Instead, each granted class is a separate row here, linked
--  back to the licence via licence_id.
--
--  HOW IT WORKS WITH vehicle_class_definitions:
--  This table stores only the class_code (e.g. 'B'). When you
--  need the human-readable description, JOIN to
--  vehicle_class_definitions ON class_code. The description lives
--  in one place; this table holds only relationships + dates.
--
--  Example for Arjun's licence:
--    Row 1: licence_id=X, class_code='B',  issued=2010-11-12, expires=2029-11-12
--    Row 2: licence_id=X, class_code='B1', issued=2010-11-12, expires=2029-11-12
--    Row 3: licence_id=X, class_code='G1', issued=2015-06-20, expires=2029-11-12
-- ────────────────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS licence_vehicle_classes (
    ...
    -- Cascade delete: removing a licence (via NIC) removes all its class rows
    FOREIGN KEY (nic) REFERENCES driving_licences(nic) ON DELETE CASCADE,
    -- Prevents invalid class codes — must exist in the definitions table
    FOREIGN KEY (class_code) REFERENCES vehicle_class_definitions(class_code),

    -- A licence/NIC cannot have the same class twice
    UNIQUE KEY uq_licence_class (nic, class_code)
);

-- Seed driving licences
...
-- Seed licence classes
INSERT INTO licence_vehicle_classes (id, nic, class_code, issued_date, expiry_date)
VALUES
('l0000000-0000-0000-0000-000000000001', '197204509123', 'B', '1995-04-10', '2032-06-15'),
('l0000000-0000-0000-0000-000000000002', '197204509123', 'A', '1995-04-10', '2032-06-15'),
('l0000000-0000-0000-0000-000000000003', '198503402948', 'B', '2003-11-15', '2029-11-15'),
('l0000000-0000-0000-0000-000000000004', '198503402948', 'A', '2003-11-15', '2029-11-15'),
('l0000000-0000-0000-0000-000000000005', '199003402948', 'B', '2008-06-01', '2030-06-01')
ON DUPLICATE KEY UPDATE nic=nic;


-- ────────────────────────────────────────────────────────────
--  TABLE: vehicle_registrations (VRC)
-- ────────────────────────────────────────────────────────────
...
-- Seed registrations (unique ID: v1, v2, v3, v4, v5)
