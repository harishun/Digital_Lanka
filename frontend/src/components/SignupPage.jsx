...
function SignupPage({ onSignupSuccess, onSwitchToLogin }) {
  const [step, setStep] = useState(1); // 1: NIC Check, 2: Full registration
  ...
  const handleNicCheck = async (e) => {
    ...
    setLoading(true);
    try {
      const response = await api.checkNic(nic.trim());
      setFullName(response.fullName);
      setInfo('NIC successfully verified with Department of Registration of Persons (DRP) and Driving Licence active.');
      setStep(2);
    } catch (err) {
