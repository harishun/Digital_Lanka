...
export default function SharedVehiclesList({ authorizedVehicles }) {
  return (
    <div className="glass-card" style={{ padding: '24px', borderRadius: '12px' }}>
      <h2 className="section-title" style={{ margin: '0 0 16px 0', fontSize: '18px', display: 'flex', alignItems: 'center', gap: '8px', color: 'var(--c-text-bright)' }}>
            ...
            >
              <div>
                ...
                <p style={{ margin: 0, fontSize: '12px', color: 'var(--c-secondary)' }}>
                  Model: <strong style={{ color: 'var(--c-card-text)' }}>{av.model || 'Toyota Prius (Grey)'}</strong> | Owner NIC: <strong style={{ color: 'var(--c-card-text)' }}>{av.ownerNic}</strong> | Type: <strong style={{ color: 'var(--c-card-text)' }}>{av.accessType}</strong>
                </p>
              </div>
