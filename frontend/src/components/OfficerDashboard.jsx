...
import NicCard from './NicCard/NicCard';
 ...
 */
function OfficerDashboard({ currentNic, currentUser, loadData, onSwitchToCitizen, embedded = false }) {
  ...
  const handleSearch = async (e) => {
    ...
    setCitationMsg('');
    try {
      // Open a 5-minute stop scoped to this plate + this driver.
      await api.openEnforcementStop(plateNo, driverNic, plateImage, currentNic);

      // Fetch the compliance record using the stop token.
      const details = await api.getEnforcementDetails(currentNic);
      setData(details);
  ...
  const handleCitation = async () => {
    ...
    try {
      const created = await api.issueEnforcementCitation({
        violationType: violationString,
        gpsCoordinates: gps,
        fineAmount: null
      }, currentNic);
      setCitationDetails({
        msg: `Citation issued successfully. Ref: ${created.referenceNumber}`,
        gps,
        violationType: violationString,
        timestamp: new Date().toLocaleString('en-US', { dateStyle: 'medium', timeStyle: 'short' })
      });
      setShowCitationModal(true);
      if (loadData) loadData();   // refresh the citizen-side inbox/citation counts
    } catch (err) {
      setCitationMsg(err.message || 'Citation failed.');
    }
  };
            ...
            <form onSubmit={handleSearch} className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div>
                ...
                <input 
                  ...
                  required
                  value={driverNic}
                  onChange={(e) => setDriverNic(e.target.value.trim())}
